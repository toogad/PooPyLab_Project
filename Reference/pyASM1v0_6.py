...
# Bulk_DO in mgO2/L
    ...
    result.append(InfComp[0] * InfComp[6] - \
                  (InfComp[0] - CompList[0]) * CompList[6] - CompList[0] * CompList[6] + \
                  0.0 * Vol)
    
    #    for C7_S_S
    result.append(InfComp[0] * InfComp[7] - \
                  (InfComp[0] - CompList[0]) * CompList[7] - CompList[0] * CompList[7] + \
                  (Stochiometrics['1_7'] * r1_AerGH(CompList, Parameters, Bulk_DO) + \
                   Stochiometrics['2_7'] * r2_AxGH(CompList, Parameters, Bulk_DO) + \
                   Stochiometrics['7_7'] * r7_HydX(CompList, Parameters, Bulk_DO))\
                  * Vol)
        
    #    for C8_S_DO
    result.append(InfComp[0] * InfComp[8] - \
                  (InfComp[0] - CompList[0]) * CompList[8] - CompList[0] * CompList[8] + \
                  (Stochiometrics['1_8'] * r1_AerGH(CompList, Parameters, Bulk_DO) + \
                   Stochiometrics['3_8'] * r3_AerGA(CompList, Parameters, Bulk_DO)) \
                  * Vol)
    #result.append(CompList[8] - Bulk_DO)
           
    #    for C9_S_NO
    result.append(InfComp[0] * InfComp[9] - \
                  (InfComp[0] - CompList[0]) * CompList[9] - CompList[0] * CompList[9] + \
    ...
    result.append(InfComp[0] * InfComp[10] - \
                  (InfComp[0] - CompList[0]) * CompList[10] - CompList[0] * CompList[10] + \
                  (Stochiometrics['1_10'] * r1_AerGH(CompList, Parameters, Bulk_DO) + \
                   Stochiometrics['2_10'] * r2_AxGH(CompList, Parameters, Bulk_DO) + \
                   Stochiometrics['3_10'] * r3_AerGA(CompList, Parameters, Bulk_DO) + \
                   Stochiometrics['6_10'] * r6_AmmSN(CompList, Parameters)) \
                  * Vol)
        
    #    for C11_S_NS
    result.append(InfComp[0] * InfComp[11] - \
                  (InfComp[0] - CompList[0]) * CompList[11] - CompList[0] * CompList[11] + \
    ...
    result.append(InfComp[0] * InfComp[12] - \
                  (InfComp[0] - CompList[0]) * 0.0 - CompList[0] * CompList[12] + \
                   (Stochiometrics['4_12'] * r4_DLH(CompList, Parameters) + \
                    Stochiometrics['5_12'] * r5_DLA(CompList, Parameters) + \
                    Stochiometrics['8_12'] * r8_HydXN(CompList, Parameters, Bulk_DO))\
                  * Vol)
            
    #    for C13_S_ALK
    result.append(InfComp[0] * InfComp[13] - \
                  (InfComp[0] - CompList[0]) * CompList[13] - CompList[0] * CompList[13] + \
                   (Stochiometrics['1_13'] * r1_AerGH(CompList, Parameters, Bulk_DO) + \
                    Stochiometrics['2_13'] * r2_AxGH(CompList, Parameters, Bulk_DO) + \
                    Stochiometrics['3_13'] * r3_AerGA(CompList, Parameters, Bulk_DO) + \
                    Stochiometrics['6_13'] * r6_AmmSN(CompList, Parameters)) \
                  * Vol)
...
MinWaterTemp = input('Minimum Water Temperature (C) = ') * 1.0
MaxWaterTemp = input('Maximum Water Temperature (C) = ') * 1.0

#Dissolved Oxygen in MLSS
Dissolved_Oxygen = input('Reactor MLSS DO (mg/L) = ') * 1.0
...
#print "Global ASM1 Parameter at Min. Water Temperature ", MinWaterTemp, " C:"
#print GP
#print "Global ASM1 Stochiometrics at Min. Water Temperature ",MinWaterTemp," C:"
#print ST

# Influent Flow, converted to M3/d    
Inf_Flow = input('Design Flow (MGD) = ') * 1000.0 * 3.78 
    
Inf_BOD5 = input('Influent BOD5 (mg/L) = ') * 1.0 #typical US Muni. WW = 240.0
#influent biodegradable COD, BOD/COD = 1.71 for typ. muni.WW
Inf_CODb = Inf_BOD5 * 1.71
#influent TSS (mgCOD/L), NOT IN InfC
Inf_TSS = input('Influent TSS (mgTSS/L) = ') * 1.0 #typ. US WW = 240.0
#influent VSS (mgCOD/L), 
Inf_VSS = 195.4 #input('Influent VSS (mgVSS/L) = ')
#influent total COD, COD/BOD5 = 2.04 per BioWin
Inf_CODt = Inf_BOD5 * 2.04 #input('Influent COD (mg/L) = ') * 1.0
...
Effluent_S_Total = input("Effluent COD (mg/L) = ") * 1.0
...
while Eff_S_S <= 0.0:
    ...
    Effluent_S_Total = input('Re-enter Eff. COD (mg/L) = ') * 1.0
    Eff_S_S = Effluent_S_Total - Inf_S_I
  
#influent TKN (mgN/L), NOT IN InfC
Inf_TKN = input('Influent TKN (mgN/L) = ') * 1.0
#influent Ammonia-N (mgN/L), 
Inf_S_NH = input('Influent Ammonia-N (mgN/L) = ') * 1.0
...
Inf_S_NO = input ('Influent Oxidized Nitrogen (NO2+NO3, mgN/L) = ') * 1.0

Effluent_Total_N = input('Effluent Total N (mgN/L) = ') * 1.0
#Assuming complete nitrification of all biodegradable TKN
#Assume 6.0% N in biomass:

if Effluent_Total_N >= Inf_TKN - Inf_nb_TKN - Eff_S_NH - GP['i_N_XD'] * \
                        (Inf_S_S + Inf_X_S - Effluent_S_Total) * GP['Y_H']:
    #which means denitrification is not required
    Eff_S_NO = Inf_S_NO + Inf_TKN - Inf_nb_TKN - Eff_S_NH - GP['i_N_XD'] * \
                (Inf_S_S + Inf_X_S) * GP['Y_H']
    ...
    S_NO_2B_DN = Inf_S_NO + Inf_TKN - Inf_nb_TKN - Eff_S_NH - GP['i_N_XD'] * \
                (Inf_S_S + Inf_X_S) * GP['Y_H'] - Eff_S_NO
...
NR_MT = 0.087 * (1.0 + GP_MT['f_D_'] * GP_MT['b_LH'] * SRT_OXIC) * GP_MT['Y_H'] / \
        (1.0 + b_H_MT * SRT_OXIC)
#autotrophic:
...
Daily_Solids_Production_Total = Daily_Solids_Production_H + \
                                Daily_Solids_Production_A + \
                                Inf_Flow * (Inf_TSS - Inf_VSS) * 1.2
...
print "Autotr. Solids Prod.= ", Daily_Solids_Production_A / 1.2 / 1000.0, " kgTSS/d"
...
MLSS = input("Design MLSS (mg/L) = ") * 1.0
...
#influent TP
Inf_TP = input('Influent TP (mgP/L) = ') * 1.0 
    
#influent Alkalinity as CaCO3 (mg/L), 
Inf_S_ALK = input('Influent Alkalinity (mmol/L) = ') * 1.0
    
#influent Dissolved Oxygen (mgO2/L), 
Inf_S_DO = input('Influent DO (mgO2/L) = ') * 1.0
...
C['3_X_BH'] = SRT_OXIC / HRT_OXIC * (InfC[7] + InfC[2] - C['7_S_S']) * GP['Y_H'] / \
                (1.0 + b_H * SRT_OXIC)
#print "X_BH = ", C['3_X_BH']
...
DesignComponents_Guess = [C['0_WAS_Flow'], \
                          ...
                          C['9_S_NO'], C['10_S_NH'], InfC[11], InfC[12], InfC[13]]
